...
class IntelligentPlanner:
    ...
    WORKFLOW_HINTS = {
        "live", "stream", "streaming", "editar", "edição", "edicao", "vídeo",
        "video", "render", "gravar", "gravação", "gravacao", "gaming", "jogar",
    }
    ...
    FOLLOW_UP_HINTS = {
        "também", "tambem", "agora", "depois", "a seguir", "o mesmo",
        "a mesma coisa", "isso", "esse", "essa", "ele", "ela", "fecha-o",
        "fecha-a", "abre-o", "abre-a",
    }

    def __init__(self, assistant, state_observer: StateObserver | None = None):
    ...
    def remember_routed_action(self, request: str, routed: dict) -> None:
        ...
        if not self._validate_arguments(tool, action, arguments):
            return
    ...
    def should_attempt(self, request: str) -> bool:
        ...
        if any(hint in text for hint in self.COMPUTER_HINTS):
            return True
        if any(hint in text for hint in self.WORKFLOW_HINTS):
            return True

        explicit_follow_up = any(hint in text for hint in self.FOLLOW_UP_HINTS)
        return bool(self._recent_plans) and explicit_follow_up

    def create_plan(self, request: str) -> Plan | None:
        if not self.should_attempt(request):
            return None

        try:
            state_snapshot = self.state_observer.observe_request(request)
        except Exception:
            state_snapshot = "(state observation unavailable)"

        prompt = self._build_prompt(request, state_snapshot)
        response = self._generate_isolated(prompt)
        data = self._extract_json(response)

        if data is None and self.MAX_REPAIR_ATTEMPTS:
            response = self._generate_isolated(
                self._build_repair_prompt(request, response)
            )
            data = self._extract_json(response)
        ...
        plan = self._build_plan(request, data)
        if plan is None:
            return None
    ...
    def _build_prompt(self, request: str, state_snapshot: str) -> str:
...
- If disk capacity matters, disk_info.info may be included so the user sees it.
- Never create a destructive action merely because of observed state.
...
- Use it only for an explicit direct follow-up such as "fecha-o", "abre também X"
  or "faz o mesmo".
- Never guess a missing target unless wording clearly refers to recent context.
- Destructive actions still require confirmation later.
...
allowed actions, or requires guessing a required path or target, return:
...
""".strip()
    ...
    def _build_repair_prompt(self, request: str, malformed_response: str) -> str:
        return f"""
...
""".strip()
    ...
    def _remember_plan(self, plan: Plan) -> None:
        actions = tuple(self._summarize_action(action) for action in plan.actions)
    ...
    def _format_recent_context(self) -> str:
        if not self._recent_plans:
            return "(none)"
        lines: list[str] = []
        for index, entry in enumerate(self._recent_plans, start=1):
            lines.append(
    ...
    def _summarize_action(action: PlanAction) -> str:
        ...
        arguments = ", ".join(
            f"{key}={value!r}" for key, value in sorted(action.arguments.items())
        )
    ...
    def _build_plan(self, request: str, data: dict[str, Any]) -> Plan | None:
        if data.get("should_plan") is not True:
            return None
        if not set(data).issubset({"should_plan", "description", "actions"}):
            return None
        ...
        for raw_action in raw_actions:
            ...
            description = raw_action.get("description", f"{tool}.{action}")
        ...
        description = data.get("description", "Plano AURA")
        if not isinstance(description, str) or not description.strip():
    ...
    def _validate_arguments(
        ...
        if tool == "process_manager" and action in {"is_running", "close"}:
            if set(arguments) != {"target"}:
                return False
            target = arguments.get("target")
            return isinstance(target, str) and bool(target.strip())
